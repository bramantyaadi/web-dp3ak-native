					...
					<nav class="navbar-left">
						<ul class="nav navbar-nav main-nav">
							...
							<li class="visible-sm visible-xs">
								...
								<form action="hasil-pencarian" method="post">
							...
							<!-- .mobile menu visible -->
							<?php               
							  $main=mysqli_query($koneksi, "SELECT * FROM menu WHERE id_parent=0 AND aktif='Y'");
							  while($r=mysqli_fetch_array($main)){
								$sub=mysqli_query($koneksi, "SELECT * FROM menu WHERE id_parent=$r[id_menu] AND aktif='Y'");
			...
			<div class="row">
				<div class="col-md-9">
						...
						<div class="row-fluid running-text">
							...
							<ul id="marquee1" class="marquee">
								<?php 
								$sekilas=mysqli_query($koneksi, "SELECT info FROM sekilasinfo ORDER BY id_sekilas DESC LIMIT 10");
								while($ts=mysqli_fetch_array($sekilas)){
								?>
				...
				<div class="col-md-3">
					...
					<nav class="navbar-right hidden-xs hidden-sm">
						<form class="no-margin" action="hasil-pencarian" method="POST">
